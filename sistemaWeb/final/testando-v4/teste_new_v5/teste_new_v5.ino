// testar as seguintes hipoteses, 
// a função do motor roda em um sentido quando vai de um angulo maior para um menor 
// e em outro quando vai de um menor para um maior, a pressão zero não é o menor angulo

// prioridade daqui pra baixo ==================================
//@1 testar esses angulos, e mudar o zero pra ajustar posição (subtrair 51) (X)


// adaptar e testar a adaptação dos botões
//testar qtd de repetições (pode estar com deficit de 1, mudar < para <=, se for o caso) (X)

// ver caminho caso não abra direto no wifi

// olhar o reptition e fast @2  (x)!! X
...
//  lcd.print("Test DISPLAY");  //Exibe a mensagem na segunda linha do display

//  var dos botoes e menu
...
String modos[] = {"ENDURANCE", "REPTITION", "FAST"};// chamar na tela, mudar pra chamar dentro das resp funções tbm
int modoIndex = 0;
int nivel = 0;
...
const char* ssid = "Nazide Pressus";
const char* password = ""; 
...
//  const int valoresPressao[] = {359, 320, 243, 131, 57, 47};
  const int valoresPressao[] = {359,339,264,193,117,113};
  return valoresPressao[nivel];
}

//@1
// novos angulos corrigidos
// 359, 320, 243, 131, 57, 47

//ver como fazer pra ficar nessa configuração/ no caso subtrair 51 graus do zero
...
void initializeMotors() {
  ...
  //@1
  moveMotorsToAngle(51);// adicionei pra ele ja ir pro angulo
...
//        moveMotorsToAngle(nivelPressao(0)); // Retorna ao nível 0
        // moveMotorsToAngle(nivelPressao(0));

           if(nivel_atual<4){
            moveMotorsToAngle((((nivelPressao(nivel_atual)+nivelPressao(nivel_atual+1))/2)-15));
           }else{
            moveMotorsToAngle((nivelPressao(5)+35));
           }
    ...
    switch (repetitionState) {
        ...
        case WAITING:
            if (currentTime - repetitionTimerStart >= repetitionContrctTime) { // Espera 1 segundo
                // media_angle = (nivelPressao(nivelInicial)+nivelPressao(nivelInicial-1))/2)
                if(nivel_atual<4){
                    moveMotorsToAngle((((repetitionAngle+nivelPressao(nivel_atual+1))/2)-15));
                }else{
                    moveMotorsToAngle((nivelPressao(5)+35));
                }

//                 if(nivel_atual>0){
//                    moveMotorsToAngle(((repetitionAngle+nivelPressao(nivel_atual-1))/2));
//                }else{
//                    moveMotorsToAngle(0);
//                }
                // moveMotorsToAngle(nivelPressao(0)); // Retorna ao nível 0
    ...
    switch (fastState) {
        ...
        case WAITING_FAST:
            if (currentTime - fastTimerStart >= fastContractTime) { // Espera 0,5 segundo (tempo reduzido para fast)
                ...
                if(nivel_atual<4){
                    moveMotorsToAngle(((fastAngle+nivelPressao(nivel_atual+1))/2)-15);
                }else{
                    moveMotorsToAngle((nivelPressao(5)+35));
                }

//                 if(nivel_atual>0){
//                    moveMotorsToAngle(((fastAngle+nivelPressao(nivel_atual-1))/2));
//                  }else{
//                      moveMotorsToAngle(0);
//                  }
                fastTimerStart = currentTime; // Inicia o temporizador
...
<body>
    ...
    <script>
        // (Funções JavaScript anteriores, modificadas para usar os selects)
...
void loop() {
  dnsServer.processNextRequest();
    ...
    // rotinas do menu de botoes

    unsigned long currentTimeMenu = millis(); // Captura o tempo atual

    for (int i = 0; i < 4; i++) {
        if (digitalRead(botoes[i]) == LOW) { // Botão pressionado
            if (currentTimeMenu - lastPressTime[i] > debounceTime) { // Verifica debounce
                lastPressTime[i] = currentTimeMenu; // Atualiza o tempo da última ativação
